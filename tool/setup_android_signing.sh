...
keystore="$signing_dir/iptvs-android-app-signing.jks"
if [ -e "$keystore" ]; then
  echo "Refusing to overwrite existing keystore: $keystore" >&2
  exit 1
fi

printf 'Key alias [iptvs-app]: '
read -r key_alias
key_alias=${key_alias:-iptvs-app}

default_certificate_dn='CN=iptvs, O=GCHOfficial, C=GB'
...
while true; do
  read -r -s -p 'New keystore password (16+ characters): ' password
  printf '\n'
  if [ "${#password}" -lt 16 ]; then
    echo "Use at least 16 characters."
    continue
  fi
  read -r -s -p 'Confirm keystore password: ' confirmation
  printf '\n'
...
export IPTVS_KEYTOOL_PASSWORD="$password"
keytool -genkeypair -v \
  -keystore "$keystore" \
  -storetype PKCS12 \
  -storepass:env IPTVS_KEYTOOL_PASSWORD \
  -keypass:env IPTVS_KEYTOOL_PASSWORD \
  -alias "$key_alias" \
...
echo
echo "Created Android app-signing key: $keystore"
echo "Alias: $key_alias"
echo "Certificate SHA-256: $fingerprint"
echo
echo "Back up this keystore and password in at least two encrypted offline locations."
echo "Do not commit the keystore or a base64 copy."
...
case "$configure_github" in
  y|Y|yes|YES)
    ...
    gh variable set ANDROID_RELEASE_CERT_SHA256 \
      --repo "$github_repository" --env release --body "$fingerprint"

    echo "Configured signing values in the GitHub release environment."
    echo "Open repository Settings > Environments > release and add protection rules."
    ;;
  *)
    echo "GitHub was not changed. Follow docs/android-signing.md when ready."
    ;;
esac
