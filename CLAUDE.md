...
- **The Back ladder** (`channel_list_screen` `_handleRootBack`, dispatched off `focusRouteKey(primaryFocus)` prefixes — the release-safe key, not `debugLabel`): Back never changes data or filters, it only climbs one level toward the exit. Live: deep channel list (scrolled > 1 viewport) → top of the list → category sidebar → "All channels" → tabs; media: deep grid → top of grid → tabs; search boxes → tabs. Focus on an **un-routed node** (route key `''` — toolbar/enrich button, AppBar action, category dropdown, or a transient node mid-rebuild) is *not* treated as the top: it recovers to the current tab (one rung below exit), since only the content tabs are `RoutedFocusNode`s. The app therefore exits **only** from the content tabs, on a **double-Back** inside a 2s window (first press shows a "Press Back again to exit" snackbar), stopping the preview engine on the actual exit. Fresh category selection lands focus on the routed category node via `LiveFocusCoordinator.focusCategory` (a `_reassertFocus` retry that wins the channel-list autofocus/scroll race) so Back peels immediately instead of only after you scroll into the list. Pinned end-to-end by `test/channel_list_focus_test.dart`.
