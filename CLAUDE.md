...
- **`lib/screens/`** — UI. `home_shell.dart` resolves the active source and builds its repository. The main browsing UI is split across `channel_list_screen.dart` (the screen state, tabs, toolbar, dropdowns — controller notifications rebuild scoped `ListenableBuilder` subtrees, never the whole screen), `live_tab_view.dart` (the live body: channel list, category side-pane, preview panel, catch-up + phone-preview sheets), `media_tab_view.dart` (movies/series grid, tiles, details sheet, series browser), and `live_focus_coordinator.dart` (`LiveFocusCoordinator`: the live tab's D-pad focus machinery — channel/category focus nodes + prune, wrap-contained pane routing (Up/Down stay within a pane; only Left/Right cross), a bidirectional vertical-hold lock, and off-screen-safe cross-pane landing (jump-scroll the target into the built viewport before focusing, since `requestFocus` on an unbuilt/cached node no-ops); its `live.channel.`/`live.category.`/`live.preview.` debugLabel scheme is **load-bearing routing keys**, see the class doc). `sources_screen.dart` manages provider configs (add/edit/delete/activate, plus ↑/↓ reorder persisted via `SourceStore.setAll`, and a per-source settings entry); `source_settings_screen.dart` toggles a source's categories on/off (persisted on `SourceConfig.settings`, filtered in the channel list); favorites are tagged from the per-item surfaces (live preview panel / phone preview sheet / media details sheet / the per-channel context menu) and surface as a "Favorites" entry atop each category list; live channels reporting an archive (`Channel.hasArchive`) get a **catch-up** button that lists cached past programmes (`CatchupSheet`) and plays them via `Source.resolveArchive`; `epg_grid_screen.dart` is the **TV-guide timeline** (one row per channel on a shared time axis) — it navigates with an explicit **selection-cursor** model (a single `epg.grid` focus node + a `cursorTime`), *not* Flutter geometry traversal: Left/Right step programme boundaries, Up/Down change channel while **holding the time column**, and the screen drives the pan/scroll itself, so navigation never depends on a lazy/async cell being built. The selected cell **and** its channel row are accent-highlighted (a solid accent-tinted cell fill + bold title, plus a full-row lift and an accent bar beside the channel name) so the cursor reads clearly from across the room — the fix for the "looks like nothing's selected / screen isn't working" report. Cells are deliberately lightweight (no per-cell `FocusableCard`) and horizontally virtualized to the visible window (safe precisely because they aren't focus targets); pinned by `test/epg_grid_test.dart`. `diagnostics_screen.dart` views/export the in-memory log; `profile_pick_screen.dart` is the boot-time "Who's watching?" profile picker (see "Profiles" below). Built to be usable by a TV remote's D-pad as well as touch/mouse — see "TV / remote navigation" below.
...
- **Live-tab pane containment** (`live_focus_coordinator.dart`): the wide/TV live body is two columns — the category sidebar and the preview-controls-plus-channel-list. Up/Down stay **within** a column and **wrap** at the ends; they never spill across panes (the fixed "Down at the last category jumps into the channel list" bug). Only Left/Right (and OK/Back) cross between them. Up from the first channel reaches the preview panel's routed Favorite/Catch-up controls (`live.preview.favorite`/`live.preview.catchup`); from there Up climbs out of the channel column to the toolbar's **search box** (`live.search.cell`, the one focusable directly above the preview panel) — the D-pad route up to search/tabs, not only the Back ladder. Up from those controls deliberately does *not* wrap to the last channel (which flung a long list to its bottom and stranded the Back ladder mid-list). On the narrow/phone layout (no sidebar/preview) channel-Up wraps directly to the last channel.
- **Channel context menu** (`channel_list_screen` `_showChannelMenu`, wired through `LiveTabView.onContextMenuChannel`): the in-place way to favorite the *focused* channel without scrolling up to the preview panel — a small **D-pad-navigable** dialog (Play / Add-Remove favorite / Catch-up-when-archive; the first action autofocuses and Up/Down move between them; `showDialog` restores focus to the same tile on dismiss, preserving the scroll spot). Opened by an **OK-hold** (D-pad, wide layout) or a **touch long-press** (wide layout *or* Android phone). On phones the menu also carries a **Preview** entry (the audible `PhonePreviewSheet`), since long-press now opens this menu instead of the sheet directly — nothing is lost. `_ChannelTile` owns the whole OK gesture where a menu is wired (`onContextMenu != null`, i.e. the wide layout): it consumes OK so the default `ActivateIntent` can't play on key-down, then a quick press plays on key-**up** while a 500 ms hold opens the menu — so a normal OK tap (and the TV preview/play-then-fullscreen gesture) is untouched. Keys it doesn't recognise as OK fall straight through to the default press-activation, so play/preview can never be broken by the hold logic.
- **The Back ladder** (`channel_list_screen` `_handleRootBack`, dispatched off `focusRouteKey(primaryFocus)` prefixes — the release-safe key, not `debugLabel`): Back never changes data or filters, it only climbs one level toward the exit. Live: deep channel list (scrolled > 1 viewport) → top of the list → category sidebar (the **last-focused** category) → "All channels" → tabs; the preview panel's Favorite/Catch-up controls (`live.preview.*`) peel to the category sidebar too; media: deep grid → top of grid → tabs; search boxes → tabs. Focus on an **un-routed node** (route key `''` — toolbar/enrich button, AppBar action, category dropdown, or a transient node mid-rebuild) is *not* treated as the top: it recovers to the current tab (one rung below exit), since only the content tabs are `RoutedFocusNode`s. The app therefore exits **only** from the content tabs, on a **double-Back** inside a 2s window (first press shows a "Press Back again to exit" snackbar), stopping the preview engine on the actual exit. Fresh category selection — and any cross-pane return from the channel list — lands focus on the routed category node via `LiveFocusCoordinator.focusCategory`/`focusCategoryFromChannels`, which **jump-scroll an off-screen category into the built viewport** (a bare `requestFocus` on an unbuilt/cached node no-ops — the root of the "Back does nothing after a category" bug on long sidebars) then `_reassertFocus`-retry to win the channel-list autofocus/scroll race, so Back peels immediately. Pinned end-to-end by `test/channel_list_focus_test.dart` (whose `_ManySource` gives the sidebar enough categories to actually scroll).
...
- **`lib/screens/update_flow.dart`** — `runUpdateCheck(context, manual:)` drives prompt → download →
  ...
  target on TV + aborts a slow download; remote/system Back cancels too). The public `showUpdateDialog`
  / `UpdateChoice` exist so `test/update_dialog_test.dart` can pin the autofocus behaviour.
