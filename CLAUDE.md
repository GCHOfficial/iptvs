...
- **The Back ladder** (`channel_list_screen` `_handleRootBack`, dispatched off `primaryFocus.debugLabel` prefixes): Back never changes data or filters, it only climbs one level toward the exit. Live: deep channel list (scrolled > 1 viewport) → top of the list → category sidebar → "All channels" → tabs; media: deep grid → top of grid → tabs; search boxes → tabs. From the tabs/toolbar the app exits only on a **double-Back** inside a 2s window (first press shows a "Press Back again to exit" snackbar) and stops the preview engine on the actual exit. Pinned end-to-end by `test/channel_list_focus_test.dart`.
