...
// Negative vectors: peer public keys that MUST be rejected before ECDH ever
// multiplies (off-curve, point-at-infinity encoding, out-of-range coordinate,
// compressed-point prefix, wrong length). `panel/src/crypto.js`'s
// `_decodePublic` (~line 361-375) checks the 65-byte 0x04 prefix, x < p,
// y < p, and the curve equation — this pins that check so a refactor can't
// silently drop it and reopen an invalid-curve attack. The Dart side has an
// equivalent (but not identical) case in
// test/cloud_crypto_test.dart:132-141 ("rejects an off-curve / malformed peer
// public key"); see that file for which of these cases it does and doesn't
// cover. Vectors come from test/fixtures/crypto_vectors.json
// `ecdhP256Invalid`, generated by test/tool/gen_crypto_vectors.dart — do not
// hand-edit the fixture.
for (const v of fixtures.ecdhP256Invalid) {
  test(`ecdhP256 rejects invalid peer public key: ${v.name}`, () => {
    assert.throws(
      () => ecdhSharedX(hexToBytes(v.privateKey_hex), hexToBytes(v.peerPublicKey_hex)),
      CloudCryptoError,
      v.note,
    );
  });
